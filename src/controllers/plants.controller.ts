...
export default class PlantsController {
    static async create(req: Request, res: Response) {
        ...
        const { name, description, imageUrl,type,origin } = PlantSchema.parse(req.body);
        if(!name || !description || !imageUrl || !type || !origin){
                res.status(400).json({
                error: "Bad Request",
    ...
    try {
        ...
        const plants = await prisma.plant.findMany({
            ...
            OR: [
                ...
                { type: { contains: query as string }, user: { id: user } },
...
static async types(req: Request, res: Response) {
    try {
        ...
        const types = Array.from(new Set(plants.map((plant) => plant.type)));
